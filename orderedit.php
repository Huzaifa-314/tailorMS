		...
		<div class="col-md-10 col-md-offset-1">
			...
			if ($_POST) {
				...
				if (!isset($error) || $error == 0) {
					...
					$res = $pdo->exec("UPDATE `order` SET `customer`='" . $customer . "',   `deadline`='" . $deadline . "',   `description`='" . $desc . "',`received_by`='" . $received_by . "',`amount`='" . $amount . "',`paid`='" . $paid . "',`date_received`='" . $date_received . "',`completed`='" . $completed . "',`date_collected`='" . $date_collected . "' WHERE id='" . $eid . "'");
	...
	Some Problem Occurs, Please Try Again. 
			...
			<form action="orderedit.php?id=<?php echo $eid ?>" method="post">
				...
				<div class="form-group">
					...
					<select name="customer" class="form-control" style="color:black;">
						<?php

						$ddaa = $pdo->query("SELECT id, username FROM `users` ORDER BY id");
				...
				<div class="form-group">
					...
					<div class="form-group">
						...
						<input type="date" name="deadline" style="width:200px; height: 40px;" value="<?php echo ($old['deadline']) ?>" /><br /><br />
					</div>
					...
					<div class="form-group">
						...
						<label>Fabric Received?</label><br />
						<select name="fabric" class="form-control" style="color:black;">
							<option value='Yes' <?php if ($old['fabric'] == 'Yes') echo "selected"; ?>>Yes</option>
							<option value='No' <?php if ($old['fabric'] == 'No') echo "selected"; ?>>No</option>
					...
					<div class="form-group">
						...
						<select name="completed" class="form-control" style="color:black;">
							<option value='pending' <?php if ($old['completed'] == 'pending') echo "selected"; ?>>Pending</option>
							<option value='confirmed' <?php if ($old['completed'] == 'confirmed') echo "selected"; ?>>Confirmed</option>
							<option value='completed' <?php if ($old['completed'] == 'completed') echo "selected"; ?>>Completed</option>
							<option value='canceled' <?php if ($old['completed'] == 'canceled') echo "selected"; ?>>Canceled</option>
